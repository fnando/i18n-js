...
describe I18n::JS::Exporter do
  describe '.config_file_path' do
    let(:default_path) { I18n::JS::Exporter::DEFAULT_CONFIG_PATH }
  ...
  context "exporting" do
    before do
      stub_const('I18n::JS::Exporter::DEFAULT_EXPORT_DIR_PATH', temp_path)
    end
    ...
    it "exports with multiple conditions to a JS file per available locale" do
      ...
      result = I18n::JS::Exporter.translation_segments
  ...
  context "filters" do
    it "filters translations using scope *.date.formats" do
      result = I18n::JS::Exporter.filter(translations, "*.date.formats")
    ...
    it "filters translations using scope [*.date.formats, *.number.currency.format]" do
      result = I18n::JS::Exporter.scoped_translations(["*.date.formats", "*.number.currency.format"])
    ...
    it "filters translations using multi-star scope" do
      result = I18n::JS::Exporter.scoped_translations("*.*.formats")
    ...
    it "filters translations using alternated stars" do
      result = I18n::JS::Exporter.scoped_translations("*.admin.*.title")
  ...
  context "exceptions" do
    it "does not include a key listed in the exceptions list" do
      result = I18n::JS::Exporter.scoped_translations("*", ['admin'])
    ...
    it "does not include multiple keys listed in the exceptions list" do
      result = I18n::JS::Exporter.scoped_translations("*", ['title', 'note'])
    ...
    it "does not include a key listed in the exceptions list and respecs the 'only' option" do
      result = I18n::JS::Exporter.scoped_translations("fr.*", ['date', 'time', 'number', 'show'])
  ...
  context "fallbacks" do
    subject do
      I18n::JS::Exporter.translation_segments.inject({}) do |hash, segment|
  ...
  context "namespace and pretty_print options" do
    ...
    it "exports with defined locale as fallback when enabled" do
      I18n::JS::Exporter.export
      file_should_exist "en.js"
      output = File.read(File.join(I18n::JS::Exporter.export_i18n_js_dir_path, "en.js"))
...
EOS
  ...
  context "general" do
    it "sets export directory" do
      I18n::JS::Exporter::DEFAULT_EXPORT_DIR_PATH.should eql("public/javascripts")
    end

    it "sets empty hash as configuration when no file is found" do
      I18n::JS::Exporter.config?.should eql(false)
      I18n::JS::Exporter.config.should eql({})
    end

    it "executes erb in config file" do
      set_config "erb.yml"

      config_entry = I18n::JS::Exporter.config["translations"].first
  ...
  describe "i18n.js exporting" do
    ...
    describe ".export_i18n_js with global variable" do
      before do
        ...
        I18n::JS::Exporter.export_i18n_js
      end
      ...
      context 'when .export_i18n_js_dir_path returns something' do
        ...
        it "exports the file" do
          File.should be_file(File.join(I18n::JS::Exporter.export_i18n_js_dir_path, "i18n.js"))
        end
      end
    ...
    describe '.export_i18n_js_dir_path' do
      let(:default_path) { I18n::JS::Exporter::DEFAULT_EXPORT_DIR_PATH }
      let(:new_path) { File.join("tmp", default_path) }
