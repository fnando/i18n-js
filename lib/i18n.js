...
I18n.toCurrency = function(number, options) {
	// If custom options is not provided, try to use
	// those specified on the translation backend.
	if (!options) {
		options = this.lookup("number.currency.format");
	}

	// Merge default options with custom/localized options
	options = this.optionsWithDefaults({
		unit: "$",
		precision: 2,
		format: "%u%n",
