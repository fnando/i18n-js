...
I18n.interpolate = function(message, object) {
	object = object || {};
	var regex = /\{\{(.*?)\}\}/gm;
	...
	for (var i = 0; placeholder = matches[i]; i++) {
		name = placeholder.replace(/\{\{(.*?)\}\}/sim, "$1");
		...
		regex = new RegExp(placeholder.replace(/\{/sm, "\\{").replace(/\}/sm, "\\}"));
...
I18n.translate = function(scope, object) {
	var message = this.lookup(scope);
	
	return this.interpolate(message, object);
};
...
I18n.parseDate = function(d) {
	...
	if (matches = d.toString().match(/(\d{4})-(\d{2})-(\d{2})(?: (\d{2}):(\d{2}):(\d{2}))?/)) {
		// date/time strings: yyyy-mm-dd hh:mm:ss or yyyy-mm-dd
		for (var i = 1; i <= 6; i++) {
