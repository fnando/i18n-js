...
I18n.optionsWithDefaults = function(defaults, options) {
	if (!defaults) {
		defaults = {};
	}
...
I18n.translate = function(scope, options) {
	...
	try {
		if (typeof(translation) == "object" && typeof(options.count) == "number") {
...
I18n.toNumber = function(number, options) {
	options = this.optionsWithDefaults(this.lookup("number.format"), options);
...
I18n.toCurrency = function(number, options) {
	options = this.optionsWithDefaults(this.lookup("number.currency.format"), options);
	options = this.optionsWithDefaults(this.lookup("number.format"), options);
	options = this.optionsWithDefaults({
		unit: "$",
		precision: 2,
...
I18n.toPercentage = function(number, options) {
	options = this.optionsWithDefaults(this.lookup("number.percentage.format"), options);
	options = this.optionsWithDefaults(this.lookup("number.format"), options);
	options = this.optionsWithDefaults({
		precision: 3,
		separator: ".",
...
I18n.pluralize = function(count, scope, options) {
	...
	switch(Math.abs(count)) {
		case 0:
			message = scope["zero"] || scope["none"] || scope["other"] || "missing '" + scope + ".zero' scope";
			break;
		case 1:
