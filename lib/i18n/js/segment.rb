module I18n
  module JS
    ...
    class Segment
      OPTIONS = [:namespace, :pretty_print, :js_extend, :sort_translation_keys, :locale_source].freeze
      ...
      def initialize(file, translations, options = {})
        ...
        @locale_source = options[:locale_source] || "I18n.available_locales"
      end

      # Saves JSON file containing translations
      def save!
        if @file =~ LOCALE_INTERPOLATOR
          eval(@locale_source).each do |locale|
